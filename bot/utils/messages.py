...
async def del_msg_by_db_name(chat_id: int, msg_name: str) -> None:
    custom_logger.debug(chat_id, f'<y>message: <r>{msg_name}</></>')

    if msg_name == 'last_schedule_message_id':
        ...
        msg_id = await db.get_db_data(chat_id, msg_name)

    await del_msg_by_id(chat_id, msg_id, msg_name)
